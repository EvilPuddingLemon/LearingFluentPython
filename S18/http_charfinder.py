...
def home(request):  # 一个路由处理函数，参数是一个 aiohttp.web.Request 实例
    ...
    #print(request.query)
    query = request.query.get('query', '').strip()  # 获取查询字符串，去掉首尾的空白。
    print('Query: {!r}'.format(query))  # 在服务器的控制台中记录查询。
    if query:
        """
        如果有查询字符串，从索引（index）中找到结果，使用 HTML 表格中的行渲染结果， 
        把结果赋值给 res 变量，再把状态消息赋值给 msg 变量。 
        """
        descriptions = list(index.find_descriptions(query, stop=200))
        #print(descriptions)
...
def main(address="127.0.0.1", port=8888):
    ...
    try:
        loop.run_forever()  # 运行事件循环；控制权在事件循环手上时，main 函数会在这里阻塞。
